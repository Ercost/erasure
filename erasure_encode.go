...
func NewEncoder(ep *EncoderParams) *Encoder {
	...
	C.minio_init_encoder(C.int(ep.Technique), k, m, &encode_matrix, &encode_tbls)
...
func (e *Encoder) Encode(input []byte) ([][]byte, error) {
	...
	C.ec_encode_data(C.int(chunkLen), C.int(k), C.int(m), e.encode_tbls, data, coding)
	return chunks, nil
}

func (e *Encoder) FreeUpEncoderInstance() {
	C.free(unsafe.Pointer(e.encode_matrix))
	C.free(unsafe.Pointer(e.encode_tbls))
}
